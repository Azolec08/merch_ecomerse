...
import * as addSampleActionModule from '../actions/add-sample-action';
...
describe('AddSampleForm', () => {
  const renderComponentUnderTest = () => {
    ...
    const events = {
      addSampleAction: vi.spyOn(addSampleActionModule, 'addSampleAction'),
    };
    const elements = {
  ...
  it('can fill up all fields with valid data and submit', async () => {
    ...
    expect(events.addSampleAction).toHaveBeenCalledWith(mockFormData);
  });
